...
## 2. for...ofを採用した理由
今回のYup検証では、配列を加工する目的ではないため map() ではなく for...of を採用した<br>
...
`map()` のコールバック関数に `async` を指定すると、<br>
それぞれの要素について `Promise` が返される。
```tsx
...
map + async を待つ：Promise.all() を使う

順番にawaitして、エラー発生時に止めたい：for...of が分かりやすい
```
